...
void asscustemp(employee emp,customer c[],int cust_size)
{
    int i,j=0;
    for(i=0;i<cust_size;i++)
    {
        printf("%d %d",emp.area_code,c[i].pincode);
        /*if(emp.area_code==c[i].pincode)
        {
            emp.c[j]=c[i];
            j++;
        }
        else
            printf("No delivery today");*/
    }
    buildmaxheap(emp.c,j);
    for(i=0;i<j;i++)
        print_customer(emp.c[i]);
}
